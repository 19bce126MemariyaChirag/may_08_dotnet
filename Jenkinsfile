pipeline {
    ...
    stages {
        ...
        stage('SSH to local') {
            ...
            steps {
                sshScript(sshpass -p usernotfound ssh anilkumar@192.168.228.188 'sudo docker kill $(sudo docker ps -q) ; sudo docker system prune -f ; sudo docker pull phadkesharanmatrixcomsec/pizza-rest-api:1.0  ; 
